...
interface StandorteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Standorte['fields']) => Promise<void>;
  defaultValues?: Standorte['fields'];
  anbieterprofilList: Anbieterprofil[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function StandorteDialog({ open, onClose, onSubmit, defaultValues, anbieterprofilList, enablePhotoScan = true, enablePhotoLocation = true }: StandorteDialogProps) {
  const [fields, setFields] = useState<Partial<Standorte['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'standorte');
      await onSubmit(clean as Standorte['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["anbieter"]);
        ...
        return merged as Partial<Standorte['fields']>;
      });
      if (gps) {
        setFields(f => ({ ...f, geo_koordinaten: { lat: gps.latitude, long: gps.longitude, info: geoAddr } as any }));
  ...
  const DIALOG_INTENT = defaultValues ? 'Standorte bearbeiten' : 'Standorte hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="standort_name">Standortname</Label>
            <Input
              id="standort_name"
              value={fields.standort_name ?? ''}
              onChange={e => setFields(f => ({ ...f, standort_name: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="geo_koordinaten">Standort auf der Karte</Label>
            <div className="space-y-3">
              <Button type="button" variant="outline" className="w-full" disabled={locating} onClick={() => geoLocate("geo_koordinaten")}>
                {locating ? <IconLoader2 className="h-4 w-4 mr-1.5 animate-spin" /> : <IconCrosshair className="h-4 w-4 mr-1.5" />}
                Aktuellen Standort verwenden
              </Button>
              {geoFromPhoto && fields.geo_koordinaten && (
                <p className="text-xs text-primary italic">Standort aus Foto übernommen</p>
              )}
              {fields.geo_koordinaten?.info && (
                <p className="text-sm text-muted-foreground break-words whitespace-normal">
                  {fields.geo_koordinaten.info}
                </p>
              )}
              {fields.geo_koordinaten?.lat != null && fields.geo_koordinaten?.long != null && (
                <GeoMapPicker
                  lat={fields.geo_koordinaten.lat}
                  lng={fields.geo_koordinaten.long}
                  onChange={(lat, lng) => handleMapMove("geo_koordinaten", lat, lng)}
                />
              )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Input type="number" step="any"
                      value={fields.geo_koordinaten?.lat ?? ''}
                      onChange={e => {
                        const v = e.target.value;
                        setFields(f => ({ ...f, geo_koordinaten: { ...(f.geo_koordinaten as any ?? {}), lat: v ? Number(v) : undefined } }));
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <Input type="number" step="any"
                      value={fields.geo_koordinaten?.long ?? ''}
                      onChange={e => {
                        const v = e.target.value;
                        setFields(f => ({ ...f, geo_koordinaten: { ...(f.geo_koordinaten as any ?? {}), long: v ? Number(v) : undefined } }));
                      }}
                    />
                  </div>
                </div>
