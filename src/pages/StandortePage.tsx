...
export default function StandortePage() {
  const [records, setRecords] = useState<Standorte[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Standorte | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Standorte | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Standorte | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, anbieterprofilData] = await Promise.all([
  ...
  async function handleCreate(fields: Standorte['fields']) {
    await LivingAppsService.createStandorteEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Standorte['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateStandorteEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteStandorteEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Standorte"
      subtitle={`${records.length} Standorte im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('standort_name')}>
                <span className="inline-flex items-center gap-1">
                  Standortname
                  {sortKey === 'standort_name' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('geo_koordinaten')}>
                <span className="inline-flex items-center gap-1">
                  Standort auf der Karte
                  {sortKey === 'geo_koordinaten' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell className="font-medium">{record.fields.standort_name ?? '—'}</TableCell>
                ...
                <TableCell className="max-w-[200px]"><span className="truncate block" title={record.fields.geo_koordinaten ? `${record.fields.geo_koordinaten.lat}, ${record.fields.geo_koordinaten.long}` : undefined}>{record.fields.geo_koordinaten?.info ?? (record.fields.geo_koordinaten ? `${record.fields.geo_koordinaten.lat?.toFixed(4)}, ${record.fields.geo_koordinaten.long?.toFixed(4)}` : '—')}</span></TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.beschreibung ?? '—'}</span></TableCell>
                <TableCell>{record.fields.kontakt_telefon ?? '—'}</TableCell>
                <TableCell>{record.fields.kontakt_email ?? '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Standorte. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
