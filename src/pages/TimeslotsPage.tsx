...
export default function TimeslotsPage() {
  const [records, setRecords] = useState<Timeslots[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Timeslots | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Timeslots | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Timeslots | null>(null);
  const [sortKey, setSortKey] = useState('');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc');
  const [anbieterprofilList, setAnbieterprofilList] = useState<Anbieterprofil[]>([]);
  const [standorteList, setStandorteList] = useState<Standorte[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, anbieterprofilData, standorteData] = await Promise.all([
        LivingAppsService.getTimeslots(),
        LivingAppsService.getAnbieterprofil(),
        LivingAppsService.getStandorte(),
      ]);
      setRecords(mainData);
      setAnbieterprofilList(anbieterprofilData);
      setStandorteList(standorteData);
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Timeslots['fields']) {
    await LivingAppsService.createTimeslot(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Timeslots['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTimeslot(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteTimeslot(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Timeslots"
      subtitle={`${records.length} Timeslots im System`}
      action={
    ...
    >
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anbieter')}>
                <span className="inline-flex items-center gap-1">
                  Anbieter
                  {sortKey === 'anbieter' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('max_kapazitaet')}>
                <span className="inline-flex items-center gap-1">
                  Maximale Anzahl Buchungen
                  {sortKey === 'max_kapazitaet' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('hinweise')}>
                <span className="inline-flex items-center gap-1">
                  Hinweise für Buchende
                  {sortKey === 'hinweise' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('online_termin')}>
                <span className="inline-flex items-center gap-1">
                  Online-Termin (kein Standort erforderlich)
                  {sortKey === 'online_termin' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('online_link')}>
                <span className="inline-flex items-center gap-1">
                  Online-Meeting-Link
                  {sortKey === 'online_link' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getAnbieterprofilDisplayName(record.fields.anbieter)}</span></TableCell>
                <TableCell className="font-medium">{record.fields.titel ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getStandorteDisplayName(record.fields.standort)}</span></TableCell>
                ...
                <TableCell>{record.fields.max_kapazitaet ?? '—'}</TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.online_termin ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.online_termin ? 'Ja' : 'Nein'}</span></TableCell>
                <TableCell>{record.fields.online_link ?? '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={12} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Timeslots. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <TimeslotsDialog
        ...
        anbieterprofilList={anbieterprofilList}
        standorteList={standorteList}
