...
export default function TerminbuchungPage() {
  const [records, setRecords] = useState<Terminbuchung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Terminbuchung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Terminbuchung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Terminbuchung | null>(null);
  const [sortKey, setSortKey] = useState('');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc');
  const [timeslotsList, setTimeslotsList] = useState<Timeslots[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, timeslotsData] = await Promise.all([
  ...
  async function handleCreate(fields: Terminbuchung['fields']) {
  ...
  async function handleUpdate(fields: Terminbuchung['fields']) {
    ...
    await LivingAppsService.updateTerminbuchungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteTerminbuchungEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Terminbuchung"
      subtitle={`${records.length} Terminbuchung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Terminbuchung suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('timeslot')}>
                <span className="inline-flex items-center gap-1">
                  Gewünschter Termin
                  {sortKey === 'timeslot' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('nachricht')}>
                <span className="inline-flex items-center gap-1">
                  Nachricht oder Anmerkungen
                  {sortKey === 'nachricht' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('datenschutz_zustimmung')}>
                <span className="inline-flex items-center gap-1">
                  Ich stimme der Verarbeitung meiner Daten zur Terminverwaltung zu.
                  {sortKey === 'datenschutz_zustimmung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getTimeslotsDisplayName(record.fields.timeslot)}</span></TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.datenschutz_zustimmung ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.datenschutz_zustimmung ? 'Ja' : 'Nein'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Terminbuchung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
