...
export default function AnbieterprofilPage() {
  const [records, setRecords] = useState<Anbieterprofil[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Anbieterprofil | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Anbieterprofil | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Anbieterprofil | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getAnbieterprofil());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Anbieterprofil['fields']) {
    await LivingAppsService.createAnbieterprofilEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Anbieterprofil['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateAnbieterprofilEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteAnbieterprofilEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Anbieterprofil im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Anbieterprofil suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('profilbild')}>
                <span className="inline-flex items-center gap-1">
                  Profilbild (optional)
                  {sortKey === 'profilbild' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.profilbild ? <div className="relative h-8 w-8 rounded bg-muted overflow-hidden"><div className="absolute inset-0 flex items-center justify-center"><IconFileText size={14} className="text-muted-foreground" /></div><img src={record.fields.profilbild} alt="" className="relative h-full w-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} /></div> : '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Anbieterprofil. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
